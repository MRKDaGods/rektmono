...
bool isInjectionlessSexSupported() {
#ifdef _M_X64
	return true;
#else
	return false;
...
REMOTE_FUNCTION(CustomRemoteFunction) {
	auto mbox = REMOTE_ARG(0, decltype(&MessageBoxA));
	auto text = REMOTE_ARG(1, const char*);
	auto caption = REMOTE_ARG(2, const char*);

	auto getProcId = REMOTE_ARG(3, decltype(&GetCurrentProcessId));
	DWORD pid = getProcId();

	auto wsprintfF = REMOTE_ARG(4, decltype(&wsprintfA));
	auto pidBuffer = REMOTE_ARG(5, char*);
	auto fmtString = REMOTE_ARG(6, const char*);
...
int main() {
	...
	success = callRemoteFunction(pi.hProcess, pi.hThread, CustomRemoteFunction, 
